...
class TestDocumentsRoutes:
    ...
    def test_upload_document_manager_forbidden(self, client, set_override, simple_ns):
        from app.core.dependencies import get_property_service, get_document_service, require_manager_or_above, get_storage_client

        prop_id = uuid.uuid4()
        prop = simple_ns(id=prop_id, manager_id=uuid.uuid4())

        set_override(get_property_service, lambda: simple_ns(get_property=lambda db, id: prop))
        set_override(get_document_service, lambda: simple_ns(create_document=lambda db, payload, storage_client=None, file_obj=None: None))
    ...
    def test_update_document_not_found_and_contract_branch_update_returns_none(self, client, set_override, simple_ns):
        from app.core.dependencies import get_document_service, get_property_service, get_contract_service, require_manager_or_above
        ...
        set_override(get_document_service, lambda: simple_ns(get_document=lambda db, id: simple_ns(contract_id=contract_id, property_id=None), update_document=lambda db, id, payload: None))
        set_override(get_contract_service, lambda: simple_ns(get_contract=lambda db, id: simple_ns(property_id=property_id)))
        set_override(get_property_service, lambda: simple_ns(get_property=lambda db, id: simple_ns(manager_id=manager_id)))
    ...
    def test_delete_document_not_found_and_contract_branch_forbidden_and_delete_returns_none(self, client, set_override, simple_ns):
        from app.core.dependencies import get_document_service, get_property_service, get_contract_service, require_manager_or_above
        ...
        set_override(get_document_service, lambda: simple_ns(get_document=lambda db, id: simple_ns(contract_id=contract_id, property_id=None), delete_document=lambda db, id: False))
        set_override(get_contract_service, lambda: simple_ns(get_contract=lambda db, id: simple_ns(property_id=property_id)))
...
class TestDocumentsUpload:
    ...
    def test_documents_upload_streams_to_storage(self, client, db, set_override):
        ...
        set_override(get_current_user := __import__('app.core.dependencies', fromlist=['get_current_user']).get_current_user, lambda: admin)
        set_override(get_storage_client := __import__('app.core.dependencies', fromlist=['get_storage_client']).get_storage_client, lambda: storage)
...
class TestDocumentsUploadStreaming:
    def test_documents_upload_route_streams_file_to_storage(self, client, db, set_override):
        ...
        set_override(__import__('app.core.dependencies', fromlist=['get_storage_client']).get_storage_client, lambda: storage)
        set_override(__import__('app.core.dependencies', fromlist=['get_current_user']).get_current_user, lambda: admin)
...
class TestDocumentsResourceAuth:
    def test_manager_cannot_update_and_delete_document_for_unmanaged_property(self, client, db, set_override):
        ...
        set_override(__import__('app.core.dependencies', fromlist=['get_current_user']).get_current_user, lambda: manager1)
