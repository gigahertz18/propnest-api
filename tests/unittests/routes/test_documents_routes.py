...
class TestDocumentsRoutes:
    ...
    async def test_create_document_manager_forbidden(self, client, set_override, simple_ns):
        ...
        set_override(get_document_service, lambda: simple_ns(create_document=AsyncMock(return_value=None)))
    ...
    async def test_upload_document_manager_forbidden(self, client, set_override, simple_ns):
        ...
        set_override(
            ...
            lambda: simple_ns(create_document=AsyncMock(return_value=None)),
        )
    ...
    async def test_update_document_not_found_and_contract_branch_update_returns_none(self, client, set_override, simple_ns):

        doc_id = uuid.uuid4()

        # First - not found
        set_override(get_document_service, lambda: simple_ns(get_document=AsyncMock(return_value=None)))
        set_override(require_manager_or_above, lambda: simple_ns(id=uuid.uuid4(), role=UserRole.MANAGER))
        ...
        set_override(
            get_document_service,
            lambda: simple_ns(
                get_document=AsyncMock(return_value=simple_ns(contract_id=contract_id, property_id=None)) ,
                update_document=AsyncMock(return_value=None),
            ),
        )
        set_override(
            get_contract_service,
            lambda: simple_ns(
                get_contract=AsyncMock(return_value=simple_ns(property_id=property_id)),
            ),
        )
        set_override(
            get_property_service,
            lambda: simple_ns(
                get_property=AsyncMock(return_value=simple_ns(manager_id=manager_id)),
            ),
        )
    ...
    async def test_delete_document_not_found_and_contract_branch_forbidden_and_delete_returns_none(
        self,
        client,
        set_override,
        simple_ns,
        ...
        set_override(
            get_document_service,
            lambda: simple_ns(get_document=AsyncMock(return_value=None)),
        )
        set_override(
            require_manager_or_above,
        ...
        set_override(
            get_property_service,
            lambda: simple_ns(
                get_property=AsyncMock(return_value=simple_ns(manager_id=uuid.uuid4())),
            ),
        )
    ...
    async def test_get_document_not_found_404(self, client, set_override, simple_ns):
...
class TestDocumentsUpload:
    ...
    async def test_documents_upload_streams_to_storage(self, client, db, set_override):
...
class TestDocumentsUploadStreaming:
    async def test_documents_upload_route_streams_file_to_storage(self, client, db, set_override):
...
class TestDocumentsResourceAuth:
    async def test_manager_cannot_update_and_delete_document_for_unmanaged_property(self, client, db, set_override):
        ...
        response = await client.patch(f"/api/v1/documents/{doc.id}", json=update_payload)
        assert response.status_code == 403
